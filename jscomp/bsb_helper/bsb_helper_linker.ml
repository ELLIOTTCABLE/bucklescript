...
(* The linker is called with object files (.cmo / .cmx) which will be namespaced
   ...
   namespaced. So we strip the namespace before reading them in. *)
...
let link link_byte_or_native ~main_module ~batch_files ~includes
  ...
  let module_to_filepath =
    Ext_list.fold_left batch_files Map_string.empty (fun m v ->
        Map_string.add m
  ...
  in
  let warning_command =
    if String.length warnings > 0 then [ "-w"; warnings ] else []
  in
  let warning_command =
    if String.length warn_error > 0 then
      "-warn-error" :: warn_error :: warning_command
    else warning_command
  in

  let tasks =
    Bsb_helper_dep_graph.simple_collect_from_main dependency_graph main_module
  in
  let namespace =
  ...
  if list_of_object_files <> [] then (
    ...
    let all_object_files =
      ...
      @ List.rev (list_of_object_files @ otherlibs)
    in
    let compiler_extension =
      if Ext_sys.is_windows_or_cygwin then ".opt.exe" else ".opt"
    in
    let local_compiler =
      (Bsb_global_paths.ocaml_dir // "bin" // compiler) ^ compiler_extension
    in
    let super_errors = if false then [ "-bs-super-errors" ] else [] in
    let list_of_args =
      (local_compiler :: "-g" :: warning_command)
      @ super_errors
      @ ("-o" :: output_file :: all_object_files)
    in
    if verbose then
      print_endline
        ("Bsb_helper link command:\n" ^ String.concat "  " list_of_args ^ "\n");
